class Sale < ApplicationRecord
  ...
  after_save do
    ...
    # Verificar se existe um desconto e aplica caso exista
    if self.discount
    ...
    # Verifica se já existe uma comissão, caso sim atualiza, caso não, criar uma nova.
    if self.comission.present?
      self.comission.update(value: (calc * 0.1), status: pending)
    else
      Comission.create(value: calc, user: self.user, sale: self, status: pending)
    end
  end
end
